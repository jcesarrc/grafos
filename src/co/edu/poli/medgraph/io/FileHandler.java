...
import co.edu.poli.medgraph.MedGraph;
...
public class FileHandler {
    ...
    public static boolean saveGraphToFile(IGraph graph, Transformer<INode, Point2D> layout, boolean dirty) {
        if (dirty && graph != null && graph.getNumberOfNodes() > 0) {
            int response = JOptionPane.showConfirmDialog(Application.getInstance(MedGraph.class).getMainFrame(), SC.t("save_graph_question"), null, JOptionPane.YES_NO_CANCEL_OPTION, JOptionPane.QUESTION_MESSAGE);
            switch (response) {
    ...
    public static void openFile(IGraph oldGraph, Transformer<INode, Point2D> oldLayout, File file, boolean dirty) {
        if (!saveGraphToFile(oldGraph, oldLayout, dirty)) {
            return;
        }
        ...
        if (file == null) {
            switch (jfc.showOpenDialog(Application.getInstance(MedGraph.class).getMainFrame())) {
        ...
        try {
            ...
            final Map<Integer, INode> nodes = new HashMap<>();
            int max = 0;
            ...
            e.printStackTrace();
            JOptionPane.showMessageDialog(Application.getInstance(MedGraph.class).getMainFrame(), SC.t("error_file_load"), SC.t("error"), JOptionPane.ERROR_MESSAGE);
        } finally {
            try {
                if (zip != null) {
                    zip.close();
    ...
    public static boolean saveFile(IGraph graph, Transformer<INode, Point2D> layout) {
        ...
        File file = null;
        switch (jfc.showSaveDialog(Application.getInstance(MedGraph.class).getMainFrame())) {
            case JFileChooser.APPROVE_OPTION:
                file = jfc.getSelectedFile();
                break;
            case JFileChooser.CANCEL_OPTION:
        ...
        try {
            ...
            final Map<Integer, Point2D> nodeLoc = new HashMap<>();
            final Map<Integer, Collection<Integer>> connections = new HashMap<>();
            final List<Integer> intermediate = new LinkedList<>();
            ...
            for (final INode v : graph.getNodes()) {
                ...
                connections.put(v.getId(), foo = new LinkedList<>());
            ...
            e.printStackTrace();
            JOptionPane.showMessageDialog(Application.getInstance(MedGraph.class).getMainFrame(), SC.t("error_file_save"), SC.t("error"), JOptionPane.ERROR_MESSAGE);
            return false;
        } finally {
            try {
                if (out != null) {
