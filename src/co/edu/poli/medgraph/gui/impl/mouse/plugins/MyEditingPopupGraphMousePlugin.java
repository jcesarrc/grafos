            ...
            if (vertex != null) {
                ...
                if (picked.size() > 1) {
                    if (graph instanceof UndirectedGraph == false) {
                        ...
                        for (final V other : picked) {
                            if (other.equals(vertex) || graph.findEdge(vertex, other) != null) {
                                continue;
                            }
                ...
                if (graph instanceof IGraph) {
                    ...
                    popup.add(new AbstractAction(SC.t("set_node_name")) {
                        @Override
                        public void actionPerformed(ActionEvent e) {
                            String name = JOptionPane.showInputDialog(Application.getInstance(MedGraph.class).getMainFrame(), SC.t("set_node_name"), v.getName());
                            if (name == null) {
